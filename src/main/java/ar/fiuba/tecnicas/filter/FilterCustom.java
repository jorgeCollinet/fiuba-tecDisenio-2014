...
public class FilterCustom implements IFilter {
	...
	public static FilterCustom generateFilterCustom(String className)
			throws Exception {
		...
		throw new Exception("nombre de clase: " + className
				+ " no implementa interfaz IFilter\n");
	}
	...
	public boolean hasToLog(FilterData filterData) {
		try {
			try {
				Constructor<?> constructor = Class.forName(this.className)
						.getConstructor();
				try {
					...
					e.printStackTrace();
					System.out
							.println("Error de programación al intentar instanciar un FilterCustom.");
					return false;
				}
