...
public class SubformatParameters 
{
	...
	private static Pattern trivialPackages = Pattern.compile("(^java\\..*)|(^sun\\..*)|(^org\\.junit\\..*)|" +
			"(^ar\\.fiuba\\.tecnicas\\..*)");
	...
	/**
	 * Constructor
	 * @param message	Mensaje				Mensaje a logear
	 * @param level		Nivel del mensaje	Nivel del mensaje
	 * @param separator	Separador 			Separador de campos
	 * @param thread	Thread				Thread desde el cual se llama
	 */
	public SubformatParameters(String message, Level level, String separator, Thread thread, String logger)
	{
	...
	{
		if (!stackWasProcessed) getStackData(thread);
		return methodName;
	}
	 ...
	 * @param element	Elemento del stack
